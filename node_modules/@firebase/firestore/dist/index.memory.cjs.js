...
function eo(t, e, n, r, i, o, s) {
    var u;
    if (i.L()) {
        if ("array-contains" /* ARRAY_CONTAINS */ === o || "array-contains-any" /* ARRAY_CONTAINS_ANY */ === o) throw new R(k.INVALID_ARGUMENT, "Invalid Query. You can't perform '" + o + "' queries on FieldPath.documentId().");
    ...
    } else "in" /* IN */ !== o && "not-in" /* NOT_IN */ !== o && "array-contains-any" /* ARRAY_CONTAINS_ANY */ !== o || io(s, o), 
    u = Si(n, e, s, 
    /* allowArrays= */ "in" /* IN */ === o || "not-in" /* NOT_IN */ === o);
    var l = fn.create(i, o, u);
    return function(t, e) {
        if (e._n()) {
            var n = t.ln();
            if (null !== n && !n.isEqual(e.field)) throw new R(k.INVALID_ARGUMENT, "Invalid query. All where filters with an inequality (<, <=, >, or >=) must be on the same field. But you have inequality filters on '" + n.toString() + "' and '" + e.field.toString() + "'");
            var r = t.cn();
            null !== r && oo(t, e.field, r);
        }
        var i = t.fn(
        /**
 ...
 */
        ...
        throw i === e.op ? new R(k.INVALID_ARGUMENT, "Invalid query. You cannot use more than one '" + e.op.toString() + "' filter.") : new R(k.INVALID_ARGUMENT, "Invalid query. You cannot use '" + e.op.toString() + "' filters with '" + i.toString() + "' filters.");
    }(t, l), l;
}

function no(t, e, n) {
    if (null !== t.startAt) throw new R(k.INVALID_ARGUMENT, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
    if (null !== t.endAt) throw new R(k.INVALID_ARGUMENT, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
    var r = new Nn(e, n);
    return function(t, e) {
        if (null === t.cn()) {
...
function ro(t, e, n) {
    if ("string" == typeof n) {
        if ("" === n) throw new R(k.INVALID_ARGUMENT, "Invalid query. When querying with FieldPath.documentId(), you must provide a valid document ID, but it was an empty string.");
        if (!Je(e) && -1 !== n.indexOf("/")) throw new R(k.INVALID_ARGUMENT, "Invalid query. When querying a collection by FieldPath.documentId(), you must provide a plain document ID, but '" + n + "' contains a '/' character.");
        var r = e.path.child(S.$(n));
        if (!U.j(r)) throw new R(k.INVALID_ARGUMENT, "Invalid query. When querying a collection group by FieldPath.documentId(), the value provided must result in a valid document path, but '" + r + "' is not because it has an odd number of segments (" + r.length + ").");
        return Ut(t, new U(r));
    }
    if (n instanceof Ii) return Ut(t, n.$c);
    throw new R(k.INVALID_ARGUMENT, "Invalid query. When querying with FieldPath.documentId(), you must provide a valid string or a DocumentReference, but it was: " + Jr(n) + ".");
}
 ...
 * array requirements.
 */ function io(t, e) {
    if (!Array.isArray(t) || 0 === t.length) throw new R(k.INVALID_ARGUMENT, "Invalid Query. A non-empty array is required for '" + e.toString() + "' filters.");
    if (t.length > 10) throw new R(k.INVALID_ARGUMENT, "Invalid Query. '" + e.toString() + "' filters support a maximum of 10 elements in the value array.");
    if ("in" /* IN */ === e || "array-contains-any" /* ARRAY_CONTAINS_ANY */ === e) {
        if (t.indexOf(null) >= 0) throw new R(k.INVALID_ARGUMENT, "Invalid Query. '" + e.toString() + "' filters cannot contain 'null' in the value array.");
        if (t.filter((function(t) {
            return Number.isNaN(t);
        })).length > 0) throw new R(k.INVALID_ARGUMENT, "Invalid Query. '" + e.toString() + "' filters cannot contain 'NaN' in the value array.");
    }
}

function oo(t, e, n) {
    if (!n.isEqual(e)) throw new R(k.INVALID_ARGUMENT, "Invalid query. You have a where filter with an inequality (<, <=, >, or >=) on field '" + e.toString() + "' and so you must also use '" + e.toString() + "' as your first orderBy(), but your first orderBy() is on field '" + n.toString() + "' instead.");
}

function so(t) {
    if (t.un() && 0 === t.en.length) throw new R(k.UNIMPLEMENTED, "limitToLast() queries require specifying at least one orderBy() clause");
}
...
var uo = /** @class */ function() {
    ...
    return e.prototype.where = function(t, n, r) {
        ...
        var i;
        Gr("Query.where", arguments, 3), ti("Query.where", 3, r), i = "not-in" === n || "!=" === n ? n : Yr("Query.where", [ "<" /* LESS_THAN */ , "<=" /* LESS_THAN_OR_EQUAL */ , "==" /* EQUAL */ , ">=" /* GREATER_THAN_OR_EQUAL */ , ">" /* GREATER_THAN */ , "array-contains" /* ARRAY_CONTAINS */ , "in" /* IN */ , "array-contains-any" /* ARRAY_CONTAINS_ANY */ ], 2, n);
        var o = Ci("Query.where", t), s = eo(this.Xl, "Query.where", this.firestore.Nl, this.firestore.Nc, o, i, r);
        return new e(function(t, e) {
            var n = t.filters.concat([ e ]);
            return new Ke(t.path, t.collectionGroup, t.en.slice(), n, t.limit, t.nn, t.startAt, t.endAt);
        }(this.Xl, s), this.firestore, this.kc);
    }, e.prototype.orderBy = function(t, n) {
        var r;
        if (Wr("Query.orderBy", arguments, 1, 2), $r("Query.orderBy", "non-empty string", 2, n), 
        void 0 === n || "asc" === n) r = "asc" /* ASCENDING */; else {
            if ("desc" !== n) throw new R(k.INVALID_ARGUMENT, "Function Query.orderBy() has unknown direction '" + n + "', expected 'asc' or 'desc'.");
            r = "desc" /* DESCENDING */;
        }
        var i = Ci("Query.orderBy", t), o = no(this.Xl, i, r);
        return new e(function(t, e) {
            // TODO(dimond): validate that orderBy does not list the same key twice.
            var n = t.en.concat([ e ]);
            return new Ke(t.path, t.collectionGroup, n, t.filters.slice(), t.limit, t.nn, t.startAt, t.endAt);
        }(this.Xl, o), this.firestore, this.kc);
    }, e.prototype.limit = function(t) {
        return Gr("Query.limit", arguments, 1), zr("Query.limit", "number", 1, t), ri("Query.limit", 1, t), 
        new e(nn(this.Xl, t, "F" /* First */), this.firestore, this.kc);
    }, e.prototype.limitToLast = function(t) {
        return Gr("Query.limitToLast", arguments, 1), zr("Query.limitToLast", "number", 1, t), 
        ri("Query.limitToLast", 1, t), new e(nn(this.Xl, t, "L" /* Last */), this.firestore, this.kc);
    }, e.prototype.startAt = function(t) {
        for (var n = [], r = 1; r < arguments.length; r++) n[r - 1] = arguments[r];
        Br("Query.startAt", arguments, 1);
        var i = this.Zl("Query.startAt", t, n, 
        /*before=*/ !0);
        return new e(rn(this.Xl, i), this.firestore, this.kc);
    }, e.prototype.startAfter = function(t) {
        for (var n = [], r = 1; r < arguments.length; r++) n[r - 1] = arguments[r];
        Br("Query.startAfter", arguments, 1);
        var i = this.Zl("Query.startAfter", t, n, 
        /*before=*/ !1);
        return new e(rn(this.Xl, i), this.firestore, this.kc);
    }, e.prototype.endBefore = function(t) {
        for (var n = [], r = 1; r < arguments.length; r++) n[r - 1] = arguments[r];
        Br("Query.endBefore", arguments, 1);
        var i = this.Zl("Query.endBefore", t, n, 
        /*before=*/ !0);
        return new e(on(this.Xl, i), this.firestore, this.kc);
    }, e.prototype.endAt = function(t) {
        for (var n = [], r = 1; r < arguments.length; r++) n[r - 1] = arguments[r];
        Br("Query.endAt", arguments, 1);
        var i = this.Zl("Query.endAt", t, n, 
        /*before=*/ !1);
        return new e(on(this.Xl, i), this.firestore, this.kc);
    }, e.prototype.isEqual = function(t) {
        if (!(t instanceof e)) throw ni("isEqual", "Query", 1, t);
        return this.firestore === t.firestore && sn(this.Xl, t.Xl) && this.kc === t.kc;
    }, e.prototype.withConverter = function(t) {
        return new e(this.Xl, this.firestore, t);
    }, 
    /** Helper function to create a bound from a document or fields */ e.prototype.Zl = function(e, n, r, i) {
        if (ti(e, 1, n), n instanceof Ji) return Gr(e, t.__spreadArrays([ n ], r), 1), function(t, e, n, r, i) {
            if (!r) throw new R(k.NOT_FOUND, "Can't use a DocumentSnapshot that doesn't exist for " + n + "().");
            ...
            for (var o = [], s = 0, u = tn(t); s < u.length; s++) {
                var a = u[s];
                if (a.field.L()) o.push(Ut(e, r.key)); else {
                    var c = r.field(a.field);
                    if (It(c)) throw new R(k.INVALID_ARGUMENT, 'Invalid query. You are trying to start or end a query using a document for which the field "' + a.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
                    if (null === c) {
                        var h = a.field.N();
                        throw new R(k.INVALID_ARGUMENT, "Invalid query. You are trying to start or end a query using a document for which the field '" + h + "' (used as the orderBy) does not exist.");
                    }
                    o.push(c);
                }
            }
            return new bn(o, i);
        }(this.Xl, this.firestore.Nc, e, n.Hl, i);
        var o = [ n ].concat(r);
        return function(t, e, n, r, i, o) {
            // Use explicit order by's because it has to match the query the user made
            var s = t.en;
            if (i.length > s.length) throw new R(k.INVALID_ARGUMENT, "Too many arguments provided to " + r + "(). The number of arguments must be less than or equal to the number of orderBy() clauses");
            for (var u = [], a = 0; a < i.length; a++) {
                var c = i[a];
                if (s[a].field.L()) {
                    if ("string" != typeof c) throw new R(k.INVALID_ARGUMENT, "Invalid query. Expected a string for document ID in " + r + "(), but got a " + typeof c);
                    if (!Je(t) && -1 !== c.indexOf("/")) throw new R(k.INVALID_ARGUMENT, "Invalid query. When querying a collection and ordering by FieldPath.documentId(), the value passed to " + r + "() must be a plain document ID, but '" + c + "' contains a slash.");
                    var h = t.path.child(S.$(c));
                    if (!U.j(h)) throw new R(k.INVALID_ARGUMENT, "Invalid query. When querying a collection group and ordering by FieldPath.documentId(), the value passed to " + r + "() must result in a valid document path, but '" + h + "' is not because it contains an odd number of segments.");
                    var f = new U(h);
                    u.push(Ut(e, f));
                } else {
                    var l = Si(n, r, c);
            ...
            return new bn(u, o);
        }(this.Xl, this.firestore.Nc, this.firestore.Nl, e, o, i);
    }, e.prototype.onSnapshot = function() {
        for (var t, e, n, r = this, i = [], o = 0; o < arguments.length; o++) i[o] = arguments[o];
        Wr("Query.onSnapshot", arguments, 1, 4);
        var s = {}, u = 0;
        if ("object" != typeof i[u] || xr(i[u]) || (ei("Query.onSnapshot", s = i[u], [ "includeMetadataChanges" ]), 
        ...
        } else zr("Query.onSnapshot", "function", u, i[u]), $r("Query.onSnapshot", "function", u + 1, i[u + 1]), 
        $r("Query.onSnapshot", "function", u + 2, i[u + 2]);
        var c = {
            next: function(t) {
                i[u] && i[u](new ao(r.firestore, r.Xl, t, r.kc));
            },
            error: i[u + 1],
            complete: i[u + 2]
        };
        return so(this.Xl), this.firestore.vl().listen(this.Xl, s, c);
    }, e.prototype.get = function(t) {
        var e = this;
        Wr("Query.get", arguments, 0, 1), lo("Query.get", t), so(this.Xl);
        var n = this.firestore.vl();
        return (t && "cache" === t.source ? n.ll(this.Xl) : n._l(this.Xl, t)).then((function(t) {
            return new ao(e.firestore, e.Xl, t, e.kc);
        }));
    }, e;
}(), ao = /** @class */ function() {
    function t(t, e, n, r) {
    ...
    return Object.defineProperty(t.prototype, "docs", {
        ...
        var n = this;
        Wr("QuerySnapshot.forEach", arguments, 1, 2), zr("QuerySnapshot.forEach", "function", 1, t), 
        this.e_.docs.forEach((function(r) {
            t.call(e, n.i_(r, n.metadata.fromCache, n.e_.qt.has(r.key)));
        }));
    }, Object.defineProperty(t.prototype, "query", {
        get: function() {
            return new uo(this.t_, this.Wl, this.kc);
        },
        enumerable: !1,
        configurable: !0
    }), t.prototype.docChanges = function(t) {
        t && (ei("QuerySnapshot.docChanges", t, [ "includeMetadataChanges" ]), Xr("QuerySnapshot.docChanges", "boolean", "includeMetadataChanges", t.includeMetadataChanges));
        var e = !(!t || !t.includeMetadataChanges);
        if (e && this.e_.Bt) throw new R(k.INVALID_ARGUMENT, "To include metadata changes with your document changes, you must also pass { includeMetadataChanges:true } to onSnapshot().");
    ...
    /** Check the equality. The call can be very expensive. */ t.prototype.isEqual = function(e) {
        ...
        return this.Wl === e.Wl && sn(this.t_, e.t_) && this.e_.isEqual(e.e_) && this.kc === e.kc;
    }, t.prototype.i_ = function(t, e, n) {
        return new to(this.Wl, t.key, t, e, n, this.kc);
    }, t;
}(), co = /** @class */ function(e) {
    function n(t, n, r) {
        var i = this;
        if ((i = e.call(this, Ze(t), n, r) || this).r_ = t, t.length % 2 != 1) throw new R(k.INVALID_ARGUMENT, "Invalid collection reference. Collection references must have an odd number of segments, but " + t.N() + " has " + t.length);
        return i;
    }
    return t.__extends(n, e), Object.defineProperty(n.prototype, "id", {
        ...
        get: function() {
            ...
            return t._() ? null : new Ki(new U(t), this.firestore, 
            /* converter= */ null);
        },
        enumerable: !1,
        configurable: !0
        ...
        Wr("CollectionReference.doc", arguments, 0, 1), 
        // We allow omission of 'pathString' but explicitly prohibit passing in both
        // 'undefined' and 'null'.
        0 === arguments.length && (t = m.t()), zr("CollectionReference.doc", "non-empty string", 1, t);
        var e = S.$(t);
        return Ki.ql(this.Xl.path.child(e), this.firestore, this.kc);
    }, n.prototype.add = function(t) {
        Gr("CollectionReference.add", arguments, 1), zr("CollectionReference.add", "object", 1, this.kc ? this.kc.toFirestore(t) : t);
        var e = this.doc();
...
function ho(t, e) {
    ...
    if (ei(t, e, [ "merge", "mergeFields" ]), Xr(t, "boolean", "merge", e.merge), function(t, e, n, r, i) {
        void 0 !== r && function(t, e, n, r, i) {
            if (!(r instanceof Array)) throw new R(k.INVALID_ARGUMENT, "Function " + t + "() requires its " + e + " option to be an array, but it was: " + Jr(r));
            for (var o = 0; o < r.length; ++o) if (!i(r[o])) throw new R(k.INVALID_ARGUMENT, "Function " + t + "() requires all " + e + " elements to be " + n + ", but the value at index " + o + " was: " + Jr(r[o]));
        }(t, e, n, r, i);
    }(t, "mergeFields", "a string or a FieldPath", e.mergeFields, (function(t) {
        return "string" == typeof t || t instanceof hi;
    })), void 0 !== e.mergeFields && void 0 !== e.merge) throw new R(k.INVALID_ARGUMENT, "Invalid options passed to function " + t + '(): You cannot specify both "merge" and "mergeFields".');
    return e;
}
...
function lo(t, e) {
    $r(t, "object", 1, e), e && (ei(t, e, [ "source" ]), Hr(t, 0, "source", e.source, [ "default", "server", "cache" ]));
}

function po(t, e, n) {
    if (e instanceof Ii) {
        if (e.firestore !== n) throw new R(k.INVALID_ARGUMENT, "Provided document reference is from a different Firestore instance.");
        return e;
    }
